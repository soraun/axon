...
jQuery(document).ready(function (){
    ...
    if (jQuery("#province").length>0){
        jQuery("#province").append("<option value='0' data-id='0' disabled selected>انتخاب کنید</option>")
        jQuery.each(provinces,function(index,item){
            jQuery("#province").append("<option value='"+item.name+"' data-id='"+item.id+"'>"+item.name+"</option>")
        });
    }
});

jQuery("#province").change(function (){
    let province_id=jQuery("#province").find(':selected').data("id");
    jQuery("#city").empty()
    jQuery.each(cities,function(index,item){
        if (item.province_id===province_id){
            jQuery("#city").append("<option value='"+item.name+"' data-id='"+item.id+"'>"+item.name+"</option>")
        }
    })
